...
def main():
    args = argparse.ArgumentParser(description='QB-Norm and Dynamic Inverted Softmax')
    args.add_argument("--train_captions_test_videos_path", 
            type=str, required=True,
            help="path to the similarity matrix between the captions from training and the videos from testing")
    args.add_argument("--test_captions_test_videos_path", 
            type=str, required=True,
    ...
    train_test = pkl.load(open(args["train_captions_test_videos_path"], 'rb'))
    test_test = pkl.load(open(args["test_captions_test_videos_path"], 'rb'))
